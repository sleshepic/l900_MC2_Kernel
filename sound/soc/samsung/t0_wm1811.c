					...
					struct snd_pcm_hw_params *params)
{
	...
	/* W/A : when aif2 interface is enable. if aif1 clock rate is not valid
	 *  reset aif1 clock rate to default value (0x3) */
	ret = snd_soc_read(wm8994->codec, WM8994_AIF1_RATE);
	if (0x3 != (ret & WM8994_AIF1CLK_RATE_MASK)) {
		pr_err("%s: AIF1 Rate: 0x%x. reset default value\n",
				__func__, ret);
		snd_soc_update_bits(wm8994->codec, WM8994_AIF1_RATE,
					WM8994_AIF1CLK_RATE_MASK, 0x0003);
	}
